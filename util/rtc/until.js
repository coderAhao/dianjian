...
import { notify, stopNotify } from './music.js'
...
const Utils = {
    ...
    const routeParams = oInfo ? ('?info=' + oInfo) : ''
    const realRoute = `/pages/rtc/${path}${routeParams}`
		if (path === 'rtmPage') {
			Store.recordState = 1;
      /**
       *执行try的目的是:
      如果重命名或修改了rtc文件夹，导致写死的realRoute未进行注册，路由跳转会发生错误，try一下便于错误的排查
      */
      try{
        uni.navigateTo({
          url: realRoute,
...
const RTMUtils = {
	...
	useLogin: function(code, uid) {
		if (code == 0) {
			...
			Store.recordUid = uid;
      store.commit('toggleRtmLoginStatus', 1)
			// uni.showToast({
			// 	title: 'RTM登录成功',
			// 	icon: 'success',
			// 	duration: 2000
      ...
      store.commit('toggleRtmLoginStatus', 0)
			// uni.showToast({
			// 	title: 'RTM登录失败',
			// 	icon: 'error',
			// 	duration: 2000
	...
	LocalInvitationFailure: async function(data) {
      ...
      // 注释掉下面的 无人接听时在通话界面手动挂断它
			// 关闭呼叫邀请回退到首页
			// await Utils.callInvitationPage('index', '', {
			// 	message: '呼叫邀请进程失败',
			// 	type: 'error'
			// });
   //    uni.$emit('callFailBack', {code: 4, mes: '被叫无人接听'})
		}
	},
	// 返回给主叫：被叫已收到呼叫邀请
    ...
    // uni.$emit('callFailBack', {code: 1, mes: '被叫已拒绝呼叫邀请'})
		// Store.recordPeerid = "";
		// 还原
		await Utils.restoreAll();
		if (data.response) {
        ...
        uni.$emit('callFailBack', {code: 1, mes: '被叫正在通话中'})
			} else {
				// 关闭呼叫邀请
				await Utils.callInvitationPage('index', '', {
					message: '被叫已拒绝呼叫邀请',
        ...
        //   icon: 'error'          
        // })
        uni.$emit('callFailBack', {code: 2, mes: '被叫已拒绝呼叫邀请'})
			}
		} else {
			// 关闭呼叫邀请
			await Utils.callInvitationPage('index', '', {
    ...
    const id = data.callerId
    store.commit('getPcCallerNumber', id)
    // uni.$emit('receiverPcCall', data)
		// 当前页面
		...
		// 判断当前用户是否正在通话中
    console.log(Store, data, 'datadatadatadatadatadatadata');
