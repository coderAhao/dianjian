...
<template>
	<view class="conent content_bg" :style="{width: windowWidth + 'px',height: windowHeight + 'px'}">
		<!-- 头像 -->
		<view class="">
			<image style="width: 120px;height: 120px;" src="/static/rtc/icon_head.png" mode=""></image>
		</view>
		<!-- 用户 uid 标识 -->
		...
		<view style="display: flex;flex-direction: column;align-items: center;margin: 40px 0;">
			<image style="width: 60px;height: 60px;" src="/static/rtc/animation.png" mode=""></image>
		...
		<view class="options">
			<view style="display: flex;justify-content: flex-end;padding: 0 40px;margin-bottom: 20px;">
				...
				<view class="icon" v-if="type === '被叫' && mode === '视频'" @click="switchFn">
					<image class="icon_img" src="/static/rtc/icon_switch_voice.png"></image>
			...
			<view class="type" :style="{justifyContent: type === '被叫' ? 'space-between': 'center'}">
				...
				<view class="icon" @click="cancelFn">
					<image class="icon_img" src="/static/rtc/icon_hangup.png"></image>
					<text class="icon_text">挂断</text>
				</view>
				<!-- 接听(仅被叫有效)-->
				<view v-if="type === '被叫'" class="icon" @click="acceptFn">
					<image class="icon_img" src="/static/rtc/icon_accept.png"></image>
...
<style scoped>
	.content_bg {
		background: url(/static/rtc/icon_back.png) no-repeat;
