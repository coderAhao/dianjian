  ...
  <view class="container">
    ...
    <view class="material">
      ...
      <view class="upload-area">
        <u-collapse ref="collapseRef" class="collapse" :border="false"
          :value="defaultOpen" @change="collapseChange">
          ...
          <u-collapse-item name="other">
            ...
            <view class="upload">
              <u-upload width="186rpx" height="186rpx" uploadIconColor="#01c1a9" :fileList="otherObj.fileList"
                @afterRead="file => afterRead('otherObj', file) " @delete="file => deletePic('otherObj', file)" name="other"
  ...
  export default {
    data() {
      return {
        ...
        defaultOpen:['tickets', 'safetyMeasuresPic', 'risk', 'record', 'invest','fireTicket','largeEquipment', 'other'],
        hiddenBtn: false
      }
    },
    components: {
    ...
    methods: {
      ...
      checkSelect() {
        ...
        if (this.equirpmentRadiovalue == 1 && !this.fireTicketObj.fileList.length) {
          uni.showToast({
            title: '请上传大型设备',
            icon: 'error',
            duration: 1500
      ...
      // 保存前的数据处理
      handleSaveData() {
        const picUploadVal = this.$checkPicStatus([this.safetyMeasuresPicObj.fileList,this.ticketObj.fileList,  this.discloseObj.fileList, this.prospectObj.fileList, this.fireTicketObj.fileList, this.largeEquipmentObj.fileList, this.otherObj.fileList])
        if (!picUploadVal) return
        const arr = ['ticketObj', 'safetyMeasuresPicObj', 'discloseObj', 'prospectObj', 'fireTicketObj',
          'largeEquipmentObj', 'otherObj'
        ]
        let saveData = {}
      ...
      // 提交前检测数据是否填写完整
      checkData() {
        const arr = ['ticketObj', 'safetyMeasuresPicObj', 'discloseObj', 'prospectObj']
      ...
      getFileDataDetail() {
        ...
        const objArr = ['safetyMeasuresPicObj', 'ticketObj', 'discloseObj', 'prospectObj', 'fireTicketObj',
          'largeEquipmentObj', 'otherObj'
        ]
        getFileDataDetail({
            constructionPlanId,
          ...
          .then(res => {
            if (res.code === 200) {
              ...
              setTimeout(() => {
                console.log(this.riskUploadList, 'this.riskUploadList[index]this.riskUploadList[index]');
              }, 1000)
