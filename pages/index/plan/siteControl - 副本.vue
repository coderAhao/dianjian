      ...
      <view class="edit-information">
        ...
        <view class="edit-btn">
          ...
          <u-button class="btn-text" color="#0ab8a2" @click="resetClickCall"
            v-if="projectData.type == 'START' && isMajor && (projectData.status == 'CHECK_ING' || projectData.status == 'WAIT_CHECK')">
            视频连线
          </u-button>
          ...
          <u-button class="loading-btn-text"
            v-if="(projectData.type == 'PAUSE' && isMajor && (projectData.status == 'CHECK_ING' || projectData.status == 'WAIT_CHECK')) && isLoading">
            视频连线
          </u-button>
          ...
          <u-button class="btn-text" color="#0ab8a2" @click="clickEditFinish"
            ...
            修改终结资料</u-button>
          <u-button class="btn-text" color="#0ab8a2" @click="resetClickCall"
            v-if="projectData.type == 'STOP' && isMajor && (projectData.status == 'CHECK_ING' || projectData.status == 'WAIT_CHECK')">
            视频连线
          </u-button>
        </view>
      </view>
    ...
    methods: {
      ...
      getProjectDetail() {
        ...
        getProjectDetail(constructionPlanId).then(res => {
          ...
          if (res.code === 200) {
            ...
            this.queryHasSafe({...queryObj})
            this.queryHasQuality({...queryObj})
          }
        }).catch(() => {})
      },
      ...
      getPhoneList() {
        ...
        getPhoneList(params).then(res => {
          ...
          if (res.code === 200) {
            // this.phoneList = res.data.sysUsers
            ...
            console.log(this.phoneList, 'this.phoneListthis.phoneListthis.phoneList');
            // 直接把data传给vuex 电话界面按需展示
            // this.$store.commit('showCallPageData', res.data)
          }
        }).catch(e => {
          console.log(e);
        })
