  ...
  <view class="container">
    <u-sticky>
      <NavigateBar @toGoBack="toGoBack">
        <view slot='navigatreText'>终结申请</view>
      </NavigateBar>
    </u-sticky>
    <view class="header">
      <view class="header-left">提交终结资料</view>
    ...
    <view class="material">
      ...
      <view class="upload-area">
        <u-collapse ref="collapseRef" :border="false" :value="defaultOpen" @change="collapseChange">
        <!-- 工作票 -->
        ...
        <!-- <view> -->
          <view class="title" slot="title">
            <view class="title-name">
              ...
              <view v-if="this.rejectReason && this.rejectReason.ticketOk" class="view-reject" @click.stop="clickShowModal('ticketOk')">(查看驳回)</view>
              
            </view>
            <view class="account">已上传{{ticketObj.count}}份</view>
          </view>
          <view class="upload">
            <u-upload width="186rpx" height="186rpx" uploadIconColor="#01c1a9" :fileList="ticketObj.fileList" @afterRead="file => afterRead('ticketObj', file) "
              @delete="file => deletePic('ticketObj', file)" name="1" multiple :maxCount="30">
            </u-upload>
          </view>
        <!-- </view> -->
        </u-collapse-item>
        ...
        <!-- <view> -->
          <view class="title" slot="title">
            <view class="title-name">
              ...
              <view class="star">*</view>
              <view v-if="this.rejectReason && this.rejectReason.closingMeetOk" class="view-reject" @click.stop="clickShowModal('closingMeetOk')">(查看驳回)</view>
              
            </view>
            <view class="account">已上传{{closingMeetPic.count}}份</view>
          </view>
          <view class="upload">
            <u-upload width="186rpx" height="186rpx" uploadIconColor="#01c1a9" :fileList="closingMeetPic.fileList" @afterRead="file => afterRead('closingMeetPic', file) "
        ...
        <!-- <view> -->
          <view class="title" slot="title">
            <view class="title-name">
              ...
              <view v-if="this.rejectReason && this.rejectReason.otherOk" class="view-reject" @click.stop="clickShowModal('otherOk')">(查看驳回)</view>
              
            </view>
            <view class="account">已上传{{otherObj.count}}份</view>
          </view>
          <view class="upload">
            <u-upload width="186rpx" height="186rpx" uploadIconColor="#01c1a9" :fileList="otherObj.fileList" @afterRead="file => afterRead('otherObj', file) "
              @delete="file => deletePic('otherObj', file)" name="6" multiple :maxCount="30">
            </u-upload>
          </view>
        <!-- </view> -->
        </u-collapse-item>
    ...
    <view class="button-area" v-show="!hiddenBtn">
      <u-button class="save-btn" color="#0ab8a2" @click="clickSave">保存</u-button>
      <u-button class="commit-btn" :plain="true" @click="clickCommit">提交</u-button>
    </view>
    <u-modal class="modal" width="550rpx" :show="showModal" :showConfirmButton="false">
      <view class="slot-content">
        <view class="modal-title">提示</view>
        <view>您确定要提交终结申请吗？</view>
  ...
  export default {
    data() {
      return {
        ...
        rejectReason: {},
        isCommiting: true,
        defaultOpen: ['tickets', 'closingMeet', 'other'],
        hiddenBtn: false
      }
    },
    components: {
    ...
    methods: {
      ...
      // 保存前的数据处理
      handleSaveData() {
        const arr = ['ticketObj', 'closingMeetPic', 'otherObj']
        let saveData = {}
      ...
      // 点击保存回到现场管控
      clickSave() {
        const picUploadVal = this.$checkPicStatus([this.ticketObj.fileList, this.closingMeetPic.fileList, this.otherObj.fileList])
        if (!picUploadVal) return
        this.handleSaveData()
      ...
      // 提交前检测数据是否填写完整
      checkData() {
        const arr = ['ticketObj', 'closingMeetPic']
      ...
      confirmCommit() {
        ...
        console.log(this.fileData, 'this.fileDatathis.fileDatathis.fileDatathis.fileData');
      ...
      getFileDataDetail() {
        ...
        const objArr = ['ticketObj', 'closingMeetPic', 'otherObj']
        console.log(type, this.$store.state.currentProject, '888888888888888888888888888');
        getFileDataDetail({
            constructionPlanId,
            type
          })
