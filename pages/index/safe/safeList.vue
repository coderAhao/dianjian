  ...
  <view class="container">
    ...
    <u-sticky>
      ...
      <view class="search">
        <u-search placeholder="输入关键字" bgColor="#ffffff" height="80rpx" shape="square" :actionStyle='{color:"#01c1a9"}'
          ...
          :show-action="true" actionText="搜索" @custom="getSafeList" @clear="clearSearch">
        </u-search>
      </view>
    </u-sticky>
    ...
    <view class="rectify-area" v-if="showNoData">
      <view class="rectify-item" v-for="item in rectifyList" :key="item.id" @click="switchRectify(item)">
        <view class="project-desc">{{item.violationDesc}}</view>
        <view class="project-name">{{item.projectName}}</view>
        <view class="project-time">
        <view class="project-type">{{item.violationType}}</view>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 拉取列表
      getSafeList() {
        getSafeList(this.query).then(res => {
      ...
      // 跳转到历史记录
      switchHistoryList() {
        uni.$u.route('/pages/index/safe/safeHistory')
      },
      // 跳转到整改
      switchRectify(item) {
        uni.$u.route('/pages/index/safe/rectify', {
          id: item.id
        })
      },
      // 返回上一页
