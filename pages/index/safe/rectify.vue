      ...
      <view class="violation">
        ...
        <view class="detail">
          <view class="detail-item" v-if="!isSelfInspection">
            <text class="front">扣分</text>
            <text class="behind">{{rectifyDetail.deduction}}</text>
          </view>
          <view class="detail-item" v-if="!isSelfInspection">
        ...
        <u-steps :current="processDetail.length" direction="column" activeColor="#73C797">
          <u-steps-item :title="item.statusContent" v-for="item in processDetail" :key="item.moduleId"
            ...
            <view class="slot-icon" slot="icon">
              <image v-if="item.statusContent !== '驳回'" class="img" src="/static/images/icon/process_pass_icon.png"
                mode=""></image>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getRectifyDetail() {
        getRectifyDetail(this.projectId).then(res => {
          ...
          if (res.code === 200) {
            ...
            this.picLength = data.violationPicList.length
            ...
            this.typeList.push(jobNature, violationLevel, violationSubject, violationType)
          }
        }).catch(() => {})
      },
      // 拉取流程详情
      getProcessDetail() {
        const obj = {
          processModule: 'SAFETY_SUPERVISION',
