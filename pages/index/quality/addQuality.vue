  ...
  <view class="container">
    ...
    <u-sticky>
      <NavigateBar @toGoBack="toGoBack">
        ...
        <view class="delete-container" v-if="showDeleteBtn" slot='define' @click="deleteQuality">
    ...
    <view class="material">
      ...
      <view class="vio-desc">
        ...
        <u--textarea v-model.trim="defectDesc" placeholder="请输入内容"></u--textarea>
      </view>
          ...
          <u-collapse-item name="pic">
            <view class="title" slot="title">
              <view class="title-name">
                <view class="title-text">缺陷图片</view>
                <view class="star">*</view>
              </view>
              <view class="account">已上传{{defectPicObj.count}}份</view>
            </view>
            <view class="upload-container">
              <u-upload width="188rpx" height="188rpx" uploadIconColor="#01c1a9" :fileList="defectPicObj.fileList"
                @afterRead="file => afterRead('defectPicObj', file) " @delete="file => deletePic('defectPicObj', file)"
                name="safety" multiple :maxCount="10">
              </u-upload>
            </view>
          </u-collapse-item>
        </u-collapse>
    ...
    <u-modal class="modal" width="550rpx" :show="showModal" :showConfirmButton="false">
      <view class="slot-content">
        ...
        <view>{{ isDelete ? '您确定要删除质量缺陷吗？' : '您确定要提交质量缺陷吗？'}}</view>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 工作性质/缺陷类型/违章专业接口都有了数据后再请求详情
      asyncSwitch() {
        Promise.all([this.getJobNature(), this.getDefectType(), this.getViolationSubject()]).then(res => {
      ...
      getProjectDetail() {
        ...
        getProjectDetail(constructionPlanId).then(res => {
          ...
          if (res.code === 200) {
            ...
            const recTerm = 72
            const rectificationCharger = charger
      ...
      // 获取缺陷类型数据
      getDefectType() {
        return getDefectType().then(res => {
          console.log(res, '缺陷类型');
          if (res.code === 200) {
            this.defectType = res.data
          }
        }).catch(() => {})
      },
      // 获取违章专业数据
      ...
      handleSaveData() {
        ...
        this.saveData.defectDesc = this.defectDesc
        // 提取缺陷图片
        if (this.defectPicObj.fileList.length) {
          const newArr = this.defectPicObj.fileList.map(item => item.commitUrl)
          this.saveData.defectPic = newArr.toString()
        }
      },
      // 点击保存
      clickSave() {
        const picUploadVal = this.$checkPicStatus([this.defectPicObj.fileList])
        if (!picUploadVal) return
        this.handleSaveData()
        Object.assign(this.saveData, this.detailData)
        this.showSaveModal = true
        console.log(this.saveData, '保存时提交的数据');
        addOrEditQuality(this.saveData).then(res => {
          if (res.code === 200) {
      ...
      checkData() {
        ...
        if (target || !this.defectPicObj.count) return false
        return true
      },
      // 点击提交弹出弹窗
      clickCommit() {
      ...
      confirmCommit() {
        ...
        if (this.isDelete) {
          ...
          console.log(commitData, 'commitDatacommitData');
          commitQuality({
            ...commitData
          }).then(res => {
      ...
      getQualityDetail() {
        ...
        const id = this.$store.state.qualityDetailId

        console.log(id, 'id************************');
        if (id) {
          getQualityDetail(id).then(res => {
            ...
            if (res.code === 200) {
              ...
              if (data.defectType) {
                const target = this.defectType.find(item => item.dictLabel == data.defectType)
                this.textList[1].text = target.dictLabel
                this.textList[1].index = this.defectType.findIndex(item => item.dictLabel == data.defectType)
              }
              ...
              if (data.defectPicList.length) {
                ...
                newArr.forEach(item => {
                  this.downLoadFile('defectPicObj', item)
                })
              }
              if (data.defectDesc !== undefined || data.defectDesc !== null || data.defectDesc !== '') {
                this.defectDesc = data.defectDesc
              }
            }
          }).catch(() => {})
        }
